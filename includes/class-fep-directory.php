	...
	function bulk_action( $action, $ids = null ) {
		...
		switch( $action ) {
			case 'block':
				...
				$message = sprintf(_n('%s user', '%s users', $count, 'front-end-pm'), number_format_i18n($count) );
				$message .= ' ';
				$message .= __('successfully blocked.', 'front-end-pm');
			...
			case 'unblock':
				...
				$message = sprintf(_n('%s user', '%s users', $count, 'front-end-pm'), number_format_i18n($count) );
				$message .= ' ';
				$message .= __('successfully unblocked.', 'front-end-pm');
				fep_success()->add( 'success', $message );
			break;
			default:
