	...
	<div id='fep-header' class='fep-table'>
		<div>
			...
			<div><strong><?php _e("Welcome", 'front-end-pm');?>: <?php echo fep_get_userdata( $user_ID, 'display_name', 'id' ); ?></strong>
	  			<div><?php _e('You have', 'front-end-pm');?> <span class="fep_new_message_count_text"><?php printf(_n('%s unread message', '%s unread messages', $unread_count, 'front-end-pm'), number_format_i18n($unread_count) ); ?></span> <?php _e('and', 'front-end-pm');?> <span class="fep_new_announcement_count_text"><?php printf(_n('%s unread announcement', '%s unread announcements', $unread_ann_count, 'front-end-pm'), number_format_i18n($unread_ann_count) ); ?></span>
				</div>
