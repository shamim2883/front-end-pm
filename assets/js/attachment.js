var fep_Index = 1;
function fep_get_by_id( id ) {
	return document.getElementById(id);
}

function fep_create_element( name ) {
...
function fep_add_new_file_field() {
	var maximum = fep_attachment_script.maximum,
		num_img = jQuery( 'input[name="fep_upload[]"]' ).size();
	if ( ( 0 != maximum && num_img < maximum ) || 0 == maximum ) {
		var id = 'p-' + fep_Index++,
			i = fep_create_element( 'input' ),
			a = fep_create_element( 'a' ),
			d = fep_create_element( 'div' );
		...
		a.onclick = function() {
			fep_remove_element( this.getAttribute( 'divid' ) );
			return false;
		}
...
function fep_add_file_field() {
	...
	var maximum = fep_attachment_script.maximum,
		num_img = jQuery( 'input[name="fep_upload[]"]' ).size();
	if ( 0 == count && ( 0 == maximum || ( 0 != maximum && num_img < maximum ) ) ) {
...
function fep_hide_file_field() {
	var maximum = fep_attachment_script.maximum,
		num_img = jQuery( 'input[name="fep_upload[]"]' ).size();
	if ( 0 != maximum && num_img > maximum - 1 ) {
		//alert('maximum');
